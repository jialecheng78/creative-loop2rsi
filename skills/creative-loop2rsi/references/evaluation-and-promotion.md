...
不同 role id 也不等于已经完成隔离执行。能够启动 subagent 或独立任务时，必须实际使用新的执行上下文，并保存 evaluator task/thread id、输入边界和原始输出路径。只由同一执行者在同一上下文中“切换身份”，只能算自评，不能用于 L2 独立校准或 L4 held-out 晋升证据。
